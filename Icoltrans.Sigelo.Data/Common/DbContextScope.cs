namespace Icoltrans.Sigelo.Data.Common
{
    using Dapper;
    ...
    public class DbContextScope<T> : IDbContextScope<T> where T : class
    {
                                            ...
                                            int? commandTimeout = null)
        {
            ...
            connection.Open();
            var output = connection.Query<T>(storedProcedure, param: (object)param, transaction: transaction, buffered: buffered,
                commandTimeout: commandTimeout, commandType: CommandType.StoredProcedure);
            connection.Close();
            return output;
        }

        public TType QuerySP<TType>(string storedProcedure, dynamic param = null, dynamic outParam = null,
                                    SqlTransaction transaction = null, bool buffered = true,
                                    int? commandTimeout = null)
        {
            using (var conn = new SqlConnection(connectionString))
            {
                var result = conn.ExecuteScalar<TType>(storedProcedure, param: (object)param, transaction: transaction, commandTimeout: commandTimeout,
        ...
        public IEnumerable<dynamic> QuerySPMapped(string storedProcedure, dynamic param = null, dynamic outParam = null,
                                            SqlTransaction transaction = null, bool buffered = true,
                                            int? commandTimeout = null)
        {
            using (var conn = new SqlConnection(connectionString))
            {
                var result = conn.Query(storedProcedure, param: (object)param, transaction: transaction, commandTimeout: commandTimeout,
                                            commandType: CommandType.StoredProcedure);
